...
const TabCards = ({ label, title, description, logo }: TabCardsProps) => {
  return (
    <Card className=" bg-[#f2f2f2]  h-[600px] max-md:h-auto flex items-center justify-center px-10 py-4 mx-2">
      <div className="flex w-full justify-around items-center max-md:flex-col">
        <div className="flex flex-col items-center w-96 gap-y-6 max-md:px-6 px-2 ">
          <CardHeader className="text-3xl text-black/60 max-md:text-lg ">
            {label}
          </CardHeader>
          <CardTitle className="text-6xl max-md:text-3xl ">{title}</CardTitle>
        ...
        <div>
          <img src={logo} alt={title} className=" w-96 h-auto" />
        </div>
      </div>
    </Card>
  );
